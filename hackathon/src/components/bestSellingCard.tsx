...
const BestSellingCard = ({ src, alt, title, description, price, category, slug }:
  {
    src: string,
    alt: string,
    title: string,
    ...
    <div className=" max-w-[380px] h-[35rem]  mx-auto shadow-md rounded-xl overflow-hidden relative group">
      <Link href={`/${category}/${slug}`}>
        ...
        <div className='block h-[20rem] relative overflow-hidden rounded'>
          <Image src={src} alt="Best Selling Product " width={380} height={200} />
        </div>
        {/* Body Section  */}
        <div className="card-body">
          {title && (
            <h2 className="scroll-m-20 border-b text-lg text-myBlackHead line-clamp-1 font-semibold tracking-tight transition-colors first:mt-0">{title}</h2>
          )}
          {price && (
